...
/// <summary>
/// The Fluent icon set the picker offers. Each entry is an <see cref="Icon" /> INSTANCE — the
/// same object handed to the specimen, which is what lets a permalink map it back to its name.
/// </summary>
public static class FluentIconCatalogue
{
    /// <summary>The offered icons, by their type name.</summary>
    public static IReadOnlyDictionary<string, Icon> All { get; } = Build();

    private static IReadOnlyDictionary<string, Icon> Build()
    {
        var catalogue = new Dictionary<string, Icon>(StringComparer.Ordinal);
        foreach (var type in typeof(Icon).Assembly.GetTypes())
        {
            if (type.IsAbstract || !typeof(Icon).IsAssignableFrom(type)
                || type.GetConstructor(Type.EmptyTypes) is null)
            {
                continue;
            }

            if (Activator.CreateInstance(type) is Icon icon)
            {
                catalogue.TryAdd(type.Name, icon);
            }
        }
